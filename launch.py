...
def _compress_folder(machine_name: str) -> pathlib.Path:
    """Compress the CWD to the parent/source_{machine_name}.tar, return path of the tar"""

    file_path = pathlib.Path.cwd().parent / f'source_{machine_name}.tar'
...
pip install -r requirements-remote.txt --trusted-host snail.goodai.com
...
def make_experiment_group_script(command_group: List[str], group_id: int, delay: int) -> str:
    commands = " &&\n".join(command_group)
...
echo $(date) '------------------ experiment group %s finished'
    ...
    """ % (
        group_id,
        group_id * delay,  # wait delay seconds between starting the experiment groups
        group_id,
        commands,
        commands,
        group_id,
...
def make_script(commands: List[List[str]], repo_name: str, noshutdown: bool, delay: int):
    max_parallel = len(commands)

    setup = setup_script(max_parallel, repo_name)

    worker_scripts = [
        make_experiment_group_script(group, id, delay) for id, group in enumerate(commands)
    ]
...
{"""
        + setup
        + "".join(worker_scripts)
        + ("echo 'Everything finished, shutdown disabled.'" if noshutdown else shutdown_script)
        + """
} &
"""
    )
...
def upload_and_run(instance, commands: List[List[str]], config: Params, noshutdown: bool, repo_name: str, delay: int, machine_name: str):
    ...
    print(f"compressing the source code..")
    ...
    try:
        ...
        ftp_client.put(filename, f"/home/ubuntu/{filename.name}")
        ftp_client.close()
    ...
    try:
        # delete the old repo (tar merges)
        stdin, stdout, stderr = client.exec_command(f"rm -rf {repo_name}")  # non-blocking
        exit_status = stdout.channel.recv_exit_status()  # Blocking call
        if exit_status == 0:
            print(f"Folder {repo_name} Deleted")
        else:
            print("Error", exit_status)

        # untar the new repo
        stdin, stdout, stderr = client.exec_command(f"tar -xf {filename.name}")
        exit_status = stdout.channel.recv_exit_status()  # Blocking call
        print(f"decompressed")

        script = make_script(commands, config.repo_name, noshutdown, delay)
